...
void check_KEYs(int* KEY1, int* KEY2, int* counter, alt_up_parallel_port_dev* KEY_dev, alt_up_audio_dev* audio_dev)
{
	int KEY_value;
...
void echo_maker(unsigned int l_buf[], unsigned int r_buf[], unsigned int* echol_buf, unsigned int* echor_buf) {
	...
	for (i = 0; i < BUF_SIZE; ++i) {
		if (i >= ECHO_INDEX1) {
			if (i >= ECHO_INDEX1) {
				*echol_buf = l_buf[i] + ALPHA * l_buf[i - ECHO_INDEX1] + BETA * l_buf[i - ECHO_INDEX2];
				*echor_buf = r_buf[i] + ALPHA * r_buf[i - ECHO_INDEX1] + BETA * r_buf[i - ECHO_INDEX2];
			}
			else {
				*echol_buf = l_buf[i] + ALPHA * l_buf[i - ECHO_INDEX1];
				*echor_buf = r_buf[i] + ALPHA * r_buf[i - ECHO_INDEX1];
			}
		}
		else {
			*echol_buf = l_buf[i];
